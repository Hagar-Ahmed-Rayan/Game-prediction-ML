...
testdata = pd.read_csv('games-regression-test.csv')
...
meanFile.close()
testdata['Age Rating']=testdata['Age Rating'].str.removesuffix('+')
testdata
...
for i in null_cols:
    testdata[i] = testdata[i].fillna(meanDic[i]).astype(np.int64)
testdata
