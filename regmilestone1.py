...
data['Age Rating']=data['Age Rating'].str.removesuffix('+')
...
######mode
# modeDic = dict()
# null_mode_cols = ['Age Rating',]
# for i in null_mode_cols:
#     modeDic[i] = data[i].mode()
#
#     data[i] = data[i].fillna(data[i].mode()).astype(np.int64)
# modeDic
# saved = open("mode1.obj", "wb")
# pickle.dump(modeDic, saved)
# saved.close()
#############mean
meanDic = dict()
null_cols = ['Age Rating','User Rating Count','Size','Current year','Original year']
for i in null_cols:
    meanDic[i] = data[i].mean()

    data[i] = data[i].fillna(data[i].mean()).astype(np.int64)
meanDic
saved = open("means1.obj", "wb")
...
colnuames_orginal_data=list(data.columns.values)
...
pickle.dump(scaler, open("scalerReg.obj","wb"))
...
pickle.dump(cls, open('GradientBoosting_model.pkl', 'wb'))
...
prediction=cls.predict(X_test)
pickle.dump(cls, open('XGBRegressor_model.pkl', 'wb'))
total_time=end_time-Start_time
################comment out here
# counter = 0
# for name, model in models.items():
